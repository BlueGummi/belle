...
`basm` allows the user to declare *global*, *constant* values for certain numbers.

```asm
magic_number = 42
.start [300]
    mov r0, #magic_number ; will evaluate to mov r0, 42
```
...
When a subroutine is called with either the `jz @subroutine` or `jmp @subroutine` instructions, the subroutine will be replaced with the actual memory address of the subroutine in the code.
