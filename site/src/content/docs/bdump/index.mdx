...
If the build and install scripts have not been run, run 

```
make
```` 

in the `bdump/` directory to build the disassembler. Then, the diassembler can be executed by calling it with `./bdump`, or can be moved to the PATH of the machine so it can be executed without the need to directly call the binary located in the source code directory.
...
| Display only ASM           | `-o` or `--only-code`   | Boolean       |    `false`    | `-o`    |
| Disable colors         | `-c` or `--colorless`  | Boolean       |    `false`    | `-c`    |
| Output memory as hex  | `-x` or `--hex-mem`  | Boolean       |    `false`    | `-x`    |
| Output operands as hex | `-X` or `--hex`  | Boolean       |    `false`    | `-X`    |
...
Labels, however, are not preserved, as they are an abstraction at assembly level and will only appear in binary as a location that is jumped to.
...
```
bdump main --only-code --colorless > main.asm
```
