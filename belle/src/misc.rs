...
macro_rules! test_instruction {
    ($bcpu:expr, $op:ident, $arg1:expr, $arg2:expr) => {{
        ...
        let ins = match stringify!($op).to_uppercase().as_str() {
            ...
            "BZ" => BZ(parse_argument($arg1)),
            "CMP" => CMP(parse_argument($arg1), parse_argument($arg2)),
            "NAND" => NAND(parse_argument($arg1), parse_argument($arg2)),
            ...
            "BNZ" => BNZ(parse_argument($arg1)),
            "BNO" => BNO(parse_argument($arg1)),
            "BL" => BL(parse_argument($arg1)),
            "BG" => BG(parse_argument($arg1)),
            _ => panic!("cheep cheep"),
        };
...
pub fn cli_argument_check() {
    ...
    if CONFIG.compact_print && !(CONFIG.debug || CONFIG.verbose) {
        eprintln!(
            "{}",
            EmuError::Impossible(
                "Must enable debug or verbose flags with compact print".to_string()
            )
        );
        process::exit(1);
    }
